...
namespace gazebo {
class ForkLiftFreeBodyPlugin: public ModelPlugin {
...
private:
	...
	physics::JointPtr fork_shift_left, fork_shift_right, fork_lift;
...
public:
	void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
		...
		this->fork_shift_left = this->model->GetJoint("shift_left");
		this->fork_shift_right = this->model->GetJoint("shift_right");
		this->fork_lift = this->model->GetJoint("lift");


		this->forklift_joint_state = this->nh_.subscribe( "/joint_states", 200,
				&ForkLiftFreeBodyPlugin::set_joint_states, this);

		// Listen to the update event. This event is broadcast every
		// simulation iteration.
		this->updateConnection = event::Events::ConnectWorldUpdateBegin(
				boost::bind(&ForkLiftFreeBodyPlugin::OnUpdate, this, _1));
	}
...
private:
	void set_joint_states(const sensor_msgs::JointState& joint_msg) {
		...
		if(!strcmp("shift", joint_msg.name[0].c_str()))
		{
			...
			status = this->fork_shift_left->SetPosition(0,  joint_msg.position[0]);
			status = this->fork_shift_right->SetPosition(0, joint_msg.position[0]);
...
GZ_REGISTER_MODEL_PLUGIN(ForkLiftFreeBodyPlugin)
}
